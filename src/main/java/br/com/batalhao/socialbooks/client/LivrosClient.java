...
public class LivrosClient {
	...
	public List<Livro> list() {
		...
		request = RequestEntity.get(URI.create(URI_BASE)).header("Authorization", credencial).build();
	...
	public String save(Livro livro) {
		RequestEntity<Livro> request;
		request = RequestEntity.post(URI.create(URI_BASE)).header("Authorization", credencial).body(livro);

		ResponseEntity<Void> response = restTemplate.exchange(request, Void.class);
		return response.getHeaders().getLocation().toString();
	}

	public Livro find(String uri) {
		RequestEntity<Void> request;
		request = RequestEntity.get(URI.create(uri)).header("Authorization", credencial).build();
